...
public class SavasDinleyici implements Listener {
    ...
    private void mermiHasari(EntityDamageByEntityEvent olay, Projectile mermi) {
        ...
        if (sinif == Sinif.AVCI && mizrakMermisiMi(mermi)) {
    ...
    private void yakinDovus(EntityDamageByEntityEvent olay, Player saldiran) {
        ...
        switch (sinif) {
            ...
            case AVCI -> {
                ...
                carpan = mizrakMi(elde)
                        ? eklenti.getConfig().getDouble("siniflar.avci.mizrak-hasar-carpani", 1.30)
                        : eklenti.getConfig().getDouble("siniflar.avci.diger-silah-carpani", 0.85);
            }
            case SUIKASTCI -> {
    ...
    /**
     * Mizrak sayilan her sey: her tur mizrak (_SPEAR ile biten wooden, stone,
     * copper, iron, golden, diamond, netherite) ve uc disli mizrak (TRIDENT).
     *
     * Material sabiti yerine isim kontrolu yapilir; boylece ileride eklenen
     * yeni mizrak turleri kod degismeden otomatik kapsanir.
     */
    private boolean mizrakMi(Material materyal) {
        if (materyal == null) {
            return false;
        }
        String ad = materyal.name();
        return ad.endsWith("_SPEAR") || ad.equals("SPEAR") || ad.equals("TRIDENT");
    }

    /** Firlatilmis mizrak mermisi (uc disli mizrak). */
    private boolean mizrakMermisiMi(Projectile mermi) {
